...
export async function POST(req: NextRequest) {
    try {
        ...
        console.error('Shop contact OTP resend error:', error)
        return NextResponse.json(
            { success: false, error: 'Unable to resend the shop contact verification code right now. Please try again later.' },
            { status: 500 },
        )
    }
}