 ...
 *
 * Resend password-reset OTP via email. Invalidates prior code & generates a fresh one.
 */
...
export async function POST(req: NextRequest) {
    try {
        ...
        const rateCheck = await checkResendRateLimit(admin, email)
        ...
        if (sendResult.success) {
            ...
            await logNotificationEvent(admin, {
                ...
                meta: { codeId, isResend: true, notConfigured: sendResult.notConfigured === true },
                ip,
            })
        }
