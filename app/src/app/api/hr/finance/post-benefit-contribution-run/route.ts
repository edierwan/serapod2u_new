...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!runId) {
            return NextResponse.json({ success: false, error: 'Benefit contribution run ID is required' }, { status: 400 })
        }

        const amountValue = body.amount
        const amount = typeof amountValue === 'number' && Number.isFinite(amountValue) ? amountValue : null

        const { data, error } = await supabase.rpc('hr_post_benefit_contribution_run_to_gl', {
            p_run_id: runId,
            p_amount: amount
        })
        if (error) {
        ...
        console.error('Failed to post benefit contribution run to GL:', error)
