...
export async function GET() {
    try {
        ...
        console.error('Failed to list expense items:', error)
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!claimId || !expenseDate || !category || amount === undefined || amount === null) {
            return NextResponse.json({ success: false, error: 'Claim, date, category, and amount are required' }, { status: 400 })
        }
