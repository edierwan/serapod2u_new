...
export async function GET() {
    try {
        ...
        console.error('Failed to list course enrollments:', error)
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!courseId || !employeeUserId) {
            return NextResponse.json({ success: false, error: 'Course and employee are required' }, { status: 400 })
        }
