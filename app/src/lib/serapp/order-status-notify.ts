...
function formatCancelledBotMessage(orderLabel: string): string {
  return [
    `✅ **Order cancelled** · **${orderLabel}**`,
    '',
...
async function insertSerappBotStatusMessage(
  admin: Admin,
  order: {
  ...
  if (order.buyer_org_id) {
    const { data: orgChats } = await admin
      ...
      .or(`owner_org_id.eq.${order.buyer_org_id},distributor_org_id.eq.${order.buyer_org_id}`)
  ...
  if (order.created_by) {
    const { data: ownerChats } = await admin
      ...
      .eq('kind', 'assistant')
      .eq('is_archived', false)
      .eq('owner_user_id', order.created_by)
  ...
  for (const conv of targets) {
    ...
    if (msgError) {
      console.warn('[serapp] status notify insert failed', conv.id, msgError.message)
      continue
    }
...
export async function notifySerappOrderApproved(
  ...
  return insertSerappBotStatusMessage(
    admin,
    { buyer_org_id: buyerOrgId, created_by: createdBy },
    body,
    [
...
/**
 * Push a bot update when a distributor cancels a Serapp hold (e.g. from History).
 * Best-effort: never throws to the cancel UI path.
 */
export async function notifySerappOrderCancelled(
  admin: Admin,
  orderId: string,
): Promise<{ notified: number; skipped?: string }> {
  const { data: order, error: orderError } = await admin
  ...
  if (orderError) throw orderError
  if (!order) return { notified: 0, skipped: 'order_not_found' }
  if (order.status !== 'cancelled') return { notified: 0, skipped: 'not_cancelled' }

  const { data: hold } = await admin
    .from('serapp_order_holds')
    .select('id, buyer_org_id, created_by, status')
    .eq('order_id', orderId)
    .maybeSingle()
  ...
  const createdBy = hold?.created_by || order.created_by
  const orderLabel = order.display_doc_no || order.order_no || orderId
  const body = formatCancelledBotMessage(orderLabel)
