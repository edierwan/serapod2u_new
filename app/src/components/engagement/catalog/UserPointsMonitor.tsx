...
interface ConsumerUser {
  ...
  referral_phone_full?: string | null
...
const ALL_COLUMNS: ColumnDef[] = [
  ...
  { id: 'reference', label: 'Reference', defaultVisible: true, group: 'identity', sortKey: 'consumer_reference', exportFn: (u) => [u.referral_name, u.referral_phone_full || u.consumer_reference, u.referral_email].filter(Boolean).join(' | ') },
...
export function UserPointsMonitor({
  ...
  bannerDescription = 'Non-shop consumer accounts collect points through the mobile app. Shop-attached staff accounts are monitored under Shop Performance.',
  ...
  emptyDescription = 'Monitor point collections and balances for non-shop consumer accounts.',
  emptySearchDescription = 'Try adjusting your search terms. Shop-attached staff appear under Shop Performance.',
          ...
          <td key={colId} className="px-3 py-3">
            {user.consumer_reference ? (
              <>
                ...
                <div className="text-xs text-muted-foreground">{user.referral_phone_full || user.consumer_reference}</div>
                {user.referral_email && (
