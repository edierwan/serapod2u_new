...
import {
  BarChart3, RefreshCw, Loader2, Calendar, Download, Wallet, AlertTriangle,
} from 'lucide-react'
...
export default function APAgingView({ userProfile }: APAgingViewProps) {
  ...
  const loadAging = useCallback(async () => {
    try {
      setLoading(true)
      const res = await fetch(`/api/accounting/ap/aging?asAt=${asAtDate}`)
      if (res.ok) {
  ...
  const handleExportCSV = () => {
    const headers = ['Supplier', 'Bills', 'Current', '31-60', '61-90', '91-120', '120+', 'Total']
    const rows = aging.map((r) => [
      r.supplier_name, r.bill_count,
    ...
    a.href = url
    a.download = `ap-aging-${asAtDate}.csv`
    a.click()
        ...
        <CardHeader className="pb-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <CardDescription>
                Outstanding supplier balances by age bucket. Total: {formatCurrency(totals?.total || 0)} across {aging.length} suppliers.
              </CardDescription>
            </div>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16">
              <Wallet className="h-12 w-12 text-green-500 mb-3" />
              <p className="text-green-700 dark:text-green-300 font-semibold">No outstanding payables</p>
              <p className="text-sm text-muted-foreground mt-1">All supplier bills are fully paid.</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {aging.map((row) => (
                    <TableRow key={row.supplier_id}>
                      <TableCell className="font-medium text-sm">{row.supplier_name}</TableCell>
                      <TableCell className="text-center text-sm text-muted-foreground">{row.bill_count}</TableCell>
                  ...
                  {totals && (
                    <TableRow className="bg-muted/50 font-bold border-t-2">
                      ...
                      <TableCell className="text-center">{totals.bill_count}</TableCell>
          ...
          {totals && totals.days_120_plus > 0 && (
            <div className="mt-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3 flex items-start gap-2">
              ...
              <p className="text-sm text-red-700 dark:text-red-300">
                <strong>{formatCurrency(totals.days_120_plus)}</strong> outstanding over 120 days. Consider payment scheduling.
              </p>
            </div>
          )}
        </CardContent>
