...
type StockCountIssuesPanelProps = {
  ...
  /** Pre-formatted by the caller so this component stays presentational. */
  existingOpeningDraftLabel?: string
  existingOpeningDraftCreatedAtLabel?: string
  existingOpeningDraftProgressLabel?: string
...
export function StockCountIssuesPanel({
  ...
  existingOpeningDraftLabel,
  existingOpeningDraftCreatedAtLabel,
  existingOpeningDraftProgressLabel,
      ...
      <CardContent className="flex flex-wrap items-start justify-between gap-3 p-4 text-sm text-blue-950">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {existingOpeningDraftLabel && (
              <p className="mt-2 font-medium">{existingOpeningDraftLabel}</p>
            )}
            {(existingOpeningDraftCreatedAtLabel || existingOpeningDraftProgressLabel) && (
              <p className="mt-1 text-xs">
                {existingOpeningDraftCreatedAtLabel && `Created ${existingOpeningDraftCreatedAtLabel}`}
                {existingOpeningDraftCreatedAtLabel && existingOpeningDraftProgressLabel && ' · '}
                {existingOpeningDraftProgressLabel && `Progress ${existingOpeningDraftProgressLabel}`}
              </p>
            )}
          </div>
        </div>
