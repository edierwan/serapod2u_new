...
const statusStyle = (status: PasteMatchResult['status']) => ({
  ...
  duplicate: 'bg-[var(--sera-orange)]/10 text-[var(--sera-orange-deep)]',
}[status])
    ...
    <div className="space-y-4">
        ...
        {groups.map(group => {
          ...
          return (
            <button key={group} type="button" role="tab" aria-selected={activeGroup === group} onClick={() => setActiveGroup(group)}
              className={`whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium ${activeGroup === group ? 'bg-orange-50 text-orange-700 ring-1 ring-orange-300' : 'text-[var(--sera-muted)] hover:bg-[var(--sera-ink)]/[0.02]'}`}>
              {group} ({count})
            </button>
          )
        })}
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative min-w-[240px] flex-1">
          <Search className="absolute left-3 top-2.5 h-4 w-4 text-[var(--sera-muted)]/70" />
      ...
      <div className="overflow-x-auto rounded-md border">
        <table className="w-full min-w-[900px] text-sm">
          <thead className="bg-[var(--sera-ink)]/[0.03] text-left text-[var(--sera-muted)]">
          ...
          <tbody>
            ...
            {visibleVariants.length === 0 && <tr><td colSpan={7} className="px-3 py-10 text-center text-[var(--sera-muted)]/80">No variants match these filters.</td></tr>}
          </tbody>
        </table>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-3 rounded-md bg-[var(--sera-ink)]/[0.03] px-4 py-3 text-sm">
          ...
          {pasteResults.length === 0 ? (
            ...
            <div className="space-y-3">
              <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-sm"><thead><tr className="border-b text-left"><th className="p-2">Line</th><th className="p-2">Entry</th><th className="p-2">Qty</th><th className="p-2">Result</th><th className="p-2">Resolve to authorized variant</th></tr></thead><tbody>
                {pasteResults.map(result => <tr key={result.line} className="border-b align-top"><td className="p-2">{result.line}</td><td className="p-2"><div>{result.name}</div><div className="text-xs text-[var(--sera-muted)]/80">Original: {result.raw}</div></td><td className="p-2">{result.quantity ?? 'Invalid'}</td><td className="p-2"><span className={`rounded-full px-2 py-1 text-xs ${statusStyle(result.status)}`}>{statusLabel(result.status)}</span>{result.duplicateOfLine && <div className="mt-1 text-xs text-[var(--sera-muted)]/80">Duplicates line {result.duplicateOfLine}</div>}{result.status === 'suggestion' && <div className="mt-1 text-xs text-purple-700">Choose a suggestion to confirm this typo match.</div>}</td><td className="p-2">
