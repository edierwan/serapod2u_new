...
export default function ManufacturerScanView({ userProfile }: ManufacturerScanViewProps) {
  ...
  useEffect(() => {
    console.log('🟢 [COMPONENT MOUNT] ManufacturerScanView mounted')
    setIsMounted(true)

    return () => {
      console.log('🔴 [COMPONENT UNMOUNT] ManufacturerScanView unmounting - cleaning up all intervals')
      setIsMounted(false)
  ...
  useEffect(() => {
    if (currentBatchProgress && currentBatchProgress.order_id && !selectedOrder) {
      console.log('🎯 Auto-setting order context from currentBatchProgress:', currentBatchProgress.order_id)
  ...
  useEffect(() => {
    ...
    if (typeof window === 'undefined') {
      console.log('⚠️ Skipping auto-refresh - not in browser')
      return
    }
    ...
    if (!currentBatchProgress || !selectedOrder) {
      console.log('⚠️ Auto-refresh conditions not met - no batch or order')
      return
    }
    ...
    if (timeSinceLastActivity > INACTIVITY_TIMEOUT) {
      console.log('💤 User inactive for 5 minutes - pausing auto-refresh')
      return
    }
    ...
    console.log('🔄 [SMART POLLING START] Starting intelligent batch progress auto-refresh')
    ...
    const intervalId = setInterval(() => {
      ...
      if (document.visibilityState !== 'visible') {
        console.log('💤 [POLLING TICK] Page hidden - skipping refresh')
        return
      }

      console.log('🔄 [POLLING TICK] Auto-refreshing batch progress...')
      if (selectedOrder) {
        loadProgress(selectedOrder)
      }
  ...
  const loadProgress = async (orderId?: string) => {
    ...
    if (!isMounted) {
      console.log('⚠️ [loadProgress] Component unmounted - skipping API call')
      return
    }
  ...
  const handleMarkCasePerfect = async () => {
    ...
    try {
      ...
      if (workerProcessedCodes.length > 0 && successCount === 0 && duplicateCodes.length === 0 && notFoundCodes.length === 0) {
        ...
        toast({
          title: '⚠️ Cannot Use Mark Perfect',
      ...
      <CardHeader className="pb-2">
        ...
        {currentBatchProgress?.order_no && (
          <p className="text-xs text-gray-600 mt-1">
            Working on : <span className="font-semibold uppercase">{currentBatchProgress.display_doc_no || currentBatchProgress.order_no}</span>
          </p>
        )}
      </CardHeader>
      <CardContent className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <div className="mt-1 space-y-0.5 text-xs text-green-700">
                    <p>⏱️ Time taken: <span className="font-semibold">{formatProcessingDuration(markPerfectTiming.duration)}</span></p>
                    <p>📦 Cases processed: <span className="font-semibold">{markPerfectTiming.casesProcessed}</span></p>
                    <p>🔗 Codes linked: <span className="font-semibold">{markPerfectTiming.totalLinked.toLocaleString()}</span></p>
                  </div>
              ...
              >
                ...
                {orders.map(order => (
                  <option key={order.id} value={order.id}>
                    {order.display_doc_no || order.order_no} - {Array.isArray(order.organizations)
                ...
                <AlertDescription className="text-orange-900">
                  ...
                  <div className="mt-3 space-y-1 text-sm text-orange-800">
                    <p className="flex items-center gap-2">
                      <span className="text-green-600 font-bold">✓</span> You can view current batch progress
                    </p>
                    <p className="flex items-center gap-2">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col sm:flex-row gap-3 justify-end">
                ...
                <Button
                  ...
                  variant="default"
                  className={`gap-2 font-semibold shadow-md disabled:cursor-not-allowed ${currentBatchProgress?.batch_status === 'completed'
        ...
        <ModeCReverseCaseView
          ...
          onJobComplete={() => {
            if (selectedOrder) {
              ...
              setTimeout(() => {
                console.log('🔄 [Mode C Complete] First refresh - loading orders and progress...')
                loadOrders().then(() => {
                  console.log('✅ Orders loaded')
                  return loadProgress(selectedOrder)
                }).then(() => {
                  console.log('✅ Progress loaded - all refreshes complete')
                })
