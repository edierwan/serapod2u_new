...
export default function LoginPageClient({ branding, loginBanners }: LoginPageClientProps) {
      ...
      try {
        ...
        if (error) {
          ...
          const { data: profile } = await supabase
            ...
            .eq('id', user.id)
            .single()
          if ((profile as any)?.org_type_code === 'END_USER') {
            router.push('/store')
          } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orgType = (userProfile as any).org_type_code || (userProfile as any).organizations?.org_type_code
      if (orgType === 'END_USER') {
        router.push('/store')
      } else {
        router.push('/dashboard')
                ...
                {loginBanners.map((_, i) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      i === selectedIndex ? 'w-6 bg-white' : 'w-2 bg-white/40 hover:bg-white/60'
                    }`}
