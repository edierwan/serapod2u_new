...
function KpiStatusPill({ status }: { status: KpiPerformanceStatus }) {
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${STATUS_PILL_STYLE[status]}`}
    >
...
export function MonthlyKpiPerformanceReportView({
  userProfile,
  onViewChange,
}: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setRuns(runsData);
        const preferred =
          runsData.find((r) => r.status === "active") || runsData[0];
        setSelectedRunId((prev) => prev || preferred?.id || "");
      } catch (err: any) {
        if (!cancelled)
          toast({
            title: "Error",
            description: "Failed to load RoadTour Events.",
            variant: "destructive",
          });
      } finally {
    ...
    const load = async () => {
      try {
        ...
        const res = await fetch(
          `/api/roadtour/kpi/plans?org_id=${encodeURIComponent(companyId)}&roadtour_run_id=${encodeURIComponent(selectedRunId)}`,
        );
  ...
  const monthOptions = useMemo(() => {
    ...
    for (const p of reportablePlans) {
      ...
      const to = p.effective_to_month
        ? kpiMonthFromDate(p.effective_to_month)
        : null;
  ...
  const loadReport = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success)
        throw new Error(json.error || "Failed to load KPI report.");
  ...
  useEffect(() => {
    ...
    setLeaderFilter("all");
    setStatusFilter("all");
  }, [selectedMonth, selectedRunId, periodType]);
  ...
  const handleExportExcel = useCallback(async () => {
    if (!companyId || !selectedRunId || !report || report.teams.length === 0)
      return;
    try {
      setExporting("excel");
      const params = new URLSearchParams({
      ...
      if (statusFilter !== "all") params.set("status", statusFilter);
      const res = await fetch(
        `/api/roadtour/kpi/report/excel?${params.toString()}`,
      );
      if (!res.ok) {
        const json = await res.json().catch(() => ({}));
    ...
    try {
      ...
      if (exportJson.data.journal_id) {
        toast({
          ...
          description: `GL Journal ${exportJson.data.journal_id.slice(0, 8)}… created with RM ${exportJson.data.total_amount.toLocaleString()} in incentives.`,
        });
      } else {
        toast({
  ...
  }, [companyId, report, selectedMonth, selectedRunId]);
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      ...
      doc.text(
        `Anchor Month: ${monthLabel}   Period: ${report.cycle.period_label} (${report.cycle.period_type})`,
        14,
        23,
      );
      doc.text(
        "KPI attribution is based on campaign QR / selected AM at scan time.",
        14,
        28,
      );
      ...
      autoTable(doc, {
        ...
        body: report.teams.map((t) => [
          ...
          t.leader_name,
          t.member_count,
          t.team_target.toLocaleString(),
          t.actual_scans.toLocaleString(),
          `${t.achievement_percent.toFixed(1)}%`,
          `RM ${t.incentive_budget.toLocaleString()}`,
          `RM ${t.estimated_payout.toLocaleString()}`,
      ...
      autoTable(doc, {
        ...
        body: report.ams.map((a) => [
          ...
          a.am_name,
          a.team_name,
          a.assigned_target.toLocaleString(),
          a.actual_scans.toLocaleString(),
      ...
      autoTable(doc, {
        startY: (doc as any).lastAutoTable.finalY + 8,
        head: [
          ["Rank", "Campaign / Shop", "Team", "Actual Scans", "% of Total"],
        ],
        body: report.top_campaigns.map((c) => [
          c.rank,
          c.campaign_name,
          c.team_name,
          c.actual_scans.toLocaleString(),
      ...
      doc.save(
        `roadtour-${report.cycle.period_type}-kpi-${report.cycle.kpi_month}.pdf`,
      );
    } catch (err: any) {
      toast({
        title: "Export failed",
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-3 sm:p-4">
        <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-7">
          <div>
            ...
            <Select
              ...
              disabled={!hasConfiguredPlan || monthOptions.length === 0}
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={
                    hasConfiguredPlan ? "Select month" : "No KPI Plan"
                  }
                />
              </SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Select
              value={periodType}
              onValueChange={(v) => setPeriodType(v as KpiPeriodType)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div>
            ...
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={runsLoading ? "Loading…" : "Select event"}
                />
              </SelectTrigger>
              <SelectContent>
                {runs.map((r) => (
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mt-3">
          <div className="flex items-center gap-1.5 text-xs text-blue-700">
            ...
            Period auto: {period.label}. Month options are limited to configured
            KPI Plan months; selected period windows include all campaigns under
            the selected event, including those created mid-period. Multiple
            campaigns per shop are supported: each scan counts for the
            campaign/QR AM at scan time.
          </div>
      ...
      {loading && (
        <Card>
          <LoadingBlock label="Computing KPI report…" />
        </Card>
      )}
      ...
      {!loading && loadError && (
        <Card>
          <EmptyBlock
            title="Failed to load KPI report"
      ...
      {!loading &&
        ...
        !plansLoading && (
          <Card>
            ...
            />
            <div className="text-center pb-6">
              <Button onClick={() => onViewChange("roadtour-kpi-settings")}>
                Create KPI Plan
              </Button>
            </div>
          </Card>
        ...
        <>
          ...
          {hasReportData && s && s.unassigned_scans > 0 && (
            <div className="border border-amber-200 bg-amber-50 text-amber-800 text-sm rounded-md px-3 py-2">
              {formatNumber(s.unassigned_scans)} successful scans in the
              selected period belong to AMs who are not in any KPI team; they
              are excluded from team totals.
            </div>
          )}
          ...
          <div className="grid gap-3 grid-cols-2 lg:grid-cols-6">
            <KpiCard
              ...
              value={formatNumber(s!.total_team_target)}
              sub="scans"
              icon={Target}
              accent="violet"
            />
            <KpiCard
              label="Actual Scans"
              value={formatNumber(s!.actual_scans)}
              sub="scans"
              icon={Scan}
              accent="blue"
            />
            <KpiCard
              ...
              value={
                s!.total_team_target > 0
                  ? `${s!.overall_achievement_percent.toFixed(1)}%`
                  : "—"
              }
              icon={TrendingUp}
              accent="green"
            />
            <KpiCard
              label="AMs Achieved KPI"
              value={`${s!.ams_achieved} / ${s!.ams_total}`}
              icon={Users}
              accent="cyan"
            />
            <KpiCard
              label="Incentive Estimated Payout"
              value={`RM ${formatNumber(Math.round(s!.incentive_estimated_payout))}`}
              icon={Wallet}
              accent="amber"
            />
            <KpiCard
              label="Teams On Track"
              value={`${s!.teams_on_track} / ${s!.teams_total}`}
              icon={Flag}
              accent="rose"
            />
          </div>
          ...
          <div className="grid gap-4 lg:grid-cols-5">
            <Card className="lg:col-span-3">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">
                  Monthly Scan Achievement by Team
                </CardTitle>
              </CardHeader>
              <CardContent>
                {report.chart_team_achievement.length === 0 ? (
                  <EmptyBlock
                    ...
                    description="Add teams to the KPI Plan to see achievement."
                  />
                ) : (
                  <div className="h-72">
            ...
            <Card className="lg:col-span-2">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">
                  Incentive Payout Overview (Est.)
                </CardTitle>
              </CardHeader>
              <CardContent>
                {report.chart_payout_by_team.every((p) => p.payout === 0) ? (
                  <EmptyBlock
                    title="No estimated payout yet"
                    description="Payouts appear once AMs record successful scans in the selected period."
                  />
                ) : (
                  <div className="h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie
                          data={report.chart_payout_by_team.filter(
                            (p) => p.payout > 0,
                          )}
                          dataKey="payout"
                          nameKey="team_name"
          ...
          <div className="grid gap-4 lg:grid-cols-5">
                      ...
                      <TableBody>
                        {report.teams.map((t) => (
                          <TableRow key={t.team_id}>
                            ...
                            <TableCell className="text-right">
                              {formatNumber(t.team_target)}
                            </TableCell>
                            <TableCell className="text-right">
                              {formatNumber(t.actual_scans)}
                            </TableCell>
                            <TableCell className="text-right font-medium">
                              {t.achievement_percent.toFixed(1)}%
                            </TableCell>
                            <TableCell className="text-right">
                              RM {formatNumber(Math.round(t.incentive_budget))}
                            </TableCell>
                            <TableCell className="text-right">
                              RM {formatNumber(Math.round(t.estimated_payout))}
                            </TableCell>
                            <TableCell>
                              <KpiStatusPill status={t.status} />
                            </TableCell>
                          </TableRow>
                        ))}
                        <TableRow className="bg-muted/40 font-medium">
                          ...
                          <TableCell className="text-right">
                            {report.teams.reduce(
                              (sum, t) => sum + t.member_count,
                              0,
                            )}
                          </TableCell>
                          <TableCell className="text-right">
                            {formatNumber(s!.total_team_target)}
                          </TableCell>
                          <TableCell className="text-right">
                            {formatNumber(s!.actual_scans)}
                          </TableCell>
                          <TableCell className="text-right">
                            {s!.overall_achievement_percent.toFixed(1)}%
                          </TableCell>
                          ...
                          <TableCell className="text-right">
                            RM{" "}
                            {formatNumber(
                              Math.round(s!.incentive_estimated_payout),
                            )}
                          </TableCell>
                          <TableCell>—</TableCell>
                        </TableRow>
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                {report.ams.length === 0 ? (
                  <EmptyBlock
                    title="No AM assignments"
                    description="Assign AMs to teams in the KPI settings."
                  />
                ) : (
                  <div className="overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        {report.ams.map((a) => (
                          <TableRow key={a.am_user_id}>
                            ...
                            <TableCell className="text-right">
                              {formatNumber(a.assigned_target)}
                            </TableCell>
                            <TableCell className="text-right">
                              {formatNumber(a.actual_scans)}
                            </TableCell>
                            <TableCell className="text-right text-muted-foreground">
                              {a.volume_tier_rate != null &&
                              a.volume_tier_rate > 0
                                ? `RM ${a.volume_tier_rate.toFixed(2)}`
                                : "0"}
                            </TableCell>
                            <TableCell className="text-right">
                              RM {formatNumber(Math.round(a.volume_incentive))}
                            </TableCell>
                            <TableCell className="text-right font-medium">
                              {a.achievement_percent.toFixed(1)}%
                            </TableCell>
                            <TableCell className="text-right">
                              RM {formatNumber(Math.round(a.incentive_earned))}
                            </TableCell>
                            <TableCell className="text-right">
                              {a.rank}
                            </TableCell>
                            <TableCell>
                              <KpiStatusPill status={a.status} />
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
          ...
          <div className="grid gap-4 lg:grid-cols-5">
            <Card className="lg:col-span-3">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">
                  Top Contributing Campaigns / Shops (By Actual Scans)
                </CardTitle>
              </CardHeader>
              <CardContent>
                {report.top_campaigns.length === 0 ? (
                  <EmptyBlock
                    title="No scans recorded in this period"
                    description="Successful campaign QR scans will appear here."
                  />
                ) : (
                  <Table>
                    <TableHeader>
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                ...
                <div className="mt-3 rounded-md border border-blue-100 bg-blue-50/60 px-3 py-2 text-xs text-blue-800 flex items-center gap-2">
                  ...
                  Plan status: {report.cycle.status} · Period{" "}
                  {report.cycle.period_label}
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      {!loading &&
        ...
        runs.length === 0 && (
          <Card>
            <EmptyBlock
              title="No RoadTour Events"
              description="Create a RoadTour Event and campaigns before configuring KPI reporting."
            />
          </Card>
        )}
    </div>
