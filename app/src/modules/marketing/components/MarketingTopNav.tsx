...
import { Megaphone, Search, ChevronRight, ChevronDown, X, Menu as MenuIcon } from 'lucide-react'
import {
    marketingNavGroups,
    getAllMarketingNavItems,
    getMarketingBreadcrumb,
    type MarketingNavGroup,
    type MarketingNavChild,
} from '@/modules/marketing/marketingNav'
...
export default function MarketingTopNav({ currentView, onNavigate }: MarketingTopNavProps) {
    ...
    const isChildActive = useCallback(
        (child: MarketingNavChild) => child.id === currentView,
        [currentView]
    )

    const breadcrumbs = useMemo(() => getMarketingBreadcrumb(currentView), [currentView])
    const allItems = useMemo(() => getAllMarketingNavItems(), [])
        ...
        <div className="sticky top-0 z-30 bg-card border-b border-border print:hidden">
            <div className="flex items-center h-12 px-3 gap-2">
                ...
                <div ref={dropdownRef} className="hidden md:flex items-center flex-1 min-w-0 relative">
                    <nav ref={navRowRef} className="flex items-center gap-0.5 overflow-x-auto scrollbar-hide" role="menubar" aria-label="Marketing navigation">
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        active ? 'bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-200' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground',
                    ...
                    {openGroupId && (() => {
                        const group = marketingNavGroups.find((g) => g.id === openGroupId)
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                childActive ? 'bg-rose-50 text-rose-700 font-semibold dark:bg-rose-900/30 dark:text-rose-300' : 'text-foreground hover:bg-accent'
                                            )}
                                        >
                ...
                <div ref={searchRef} className="relative shrink-0">
                    {searchOpen ? (
                        <div className="flex items-center gap-1.5 bg-accent rounded-md px-2 py-1">
                            ...
                            <input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search Marketing…" className="bg-transparent text-sm outline-none w-32 sm:w-44 placeholder:text-muted-foreground"
                        ...
                        <button onClick={() => setSearchOpen(true)} className="flex items-center gap-1.5 px-2 py-1 rounded-md text-sm text-muted-foreground hover:bg-accent transition-colors" aria-label="Search Marketing">
            ...
            {mobileMenuOpen && (
                <div className="md:hidden border-t border-border bg-card max-h-[60vh] overflow-y-auto">
                    {marketingNavGroups.map((group) => {
                        ...
                        return (
                            <div key={group.id}>
                                <button onClick={() => setOpenGroupId(open ? null : group.id)} className={cn('w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium transition-colors', active ? 'bg-rose-50 text-rose-800 dark:bg-rose-900/30 dark:text-rose-200' : 'text-foreground hover:bg-accent')}>
                                ...
                                {open && (
                                    <div className="bg-accent/30">
                                        {group.children.map((child) => {
                                            ...
                                            return (<button key={child.id} onClick={() => handleNav(child.id)} className={cn('w-full flex items-center gap-2.5 pl-10 pr-4 py-2 text-sm transition-colors', childActive ? 'bg-rose-50 text-rose-700 font-semibold dark:bg-rose-900/30 dark:text-rose-300' : 'text-muted-foreground hover:bg-accent')}>
