...
import { ShoppingCart, Search, ChevronRight, ChevronDown, X, Menu as MenuIcon } from 'lucide-react'
import {
    catalogNavGroups,
    getAllCatalogNavItems,
    getCatalogBreadcrumb,
    type CatalogNavGroup,
    type CatalogNavChild,
} from '@/modules/catalog/catalogNav'
...
export default function CatalogTopNav({ currentView, onNavigate }: CatalogTopNavProps) {
    ...
    const isChildActive = useCallback(
        (child: CatalogNavChild) => child.id === currentView,
        [currentView]
    )

    const breadcrumbs = useMemo(() => getCatalogBreadcrumb(currentView), [currentView])
    const allItems = useMemo(() => getAllCatalogNavItems(), [])
        ...
        <div className="sticky top-0 z-30 bg-card border-b border-border print:hidden">
            <div className="flex items-center h-12 px-3 gap-2">
                <div className="flex items-center gap-1.5 shrink-0 mr-1">
                    <div className="flex items-center gap-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 px-2.5 py-0.5 rounded text-sm font-semibold">
                ...
                <div ref={dropdownRef} className="hidden md:flex items-center flex-1 min-w-0 relative">
                    <nav ref={navRowRef} className="flex items-center gap-0.5 overflow-x-auto scrollbar-hide" role="menubar" aria-label="Catalog navigation">
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        active ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/40 dark:text-indigo-200' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground',
                    ...
                    {openGroupId && (() => {
                        const group = catalogNavGroups.find((g) => g.id === openGroupId)
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                childActive ? 'bg-indigo-50 text-indigo-700 font-semibold dark:bg-indigo-900/30 dark:text-indigo-300' : 'text-foreground hover:bg-accent'
                                            )}
                                        >
                ...
                <div className="flex md:hidden flex-1 min-w-0">
                    <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="flex items-center gap-1.5 px-2 py-1 rounded-md text-sm font-medium text-muted-foreground hover:bg-accent" aria-label="Toggle Product Catalog menu">
                        <MenuIcon className="h-4 w-4" /><span>Product Catalog Menu</span>
                ...
                <div ref={searchRef} className="relative shrink-0">
                    {searchOpen ? (
                        <div className="flex items-center gap-1.5 bg-accent rounded-md px-2 py-1">
                            ...
                            <input autoFocus value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search Product Catalog…" className="bg-transparent text-sm outline-none w-32 sm:w-44 placeholder:text-muted-foreground"
                        ...
                        <button onClick={() => setSearchOpen(true)} className="flex items-center gap-1.5 px-2 py-1 rounded-md text-sm text-muted-foreground hover:bg-accent transition-colors" aria-label="Search Product Catalog">
            ...
            {mobileMenuOpen && (
                <div className="md:hidden border-t border-border bg-card max-h-[60vh] overflow-y-auto">
                    {catalogNavGroups.map((group) => {
                        ...
                        return (
                            <div key={group.id}>
                                <button onClick={() => setOpenGroupId(open ? null : group.id)} className={cn('w-full flex items-center gap-2.5 px-4 py-2.5 text-sm font-medium transition-colors', active ? 'bg-indigo-50 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-200' : 'text-foreground hover:bg-accent')}>
                                ...
                                {open && (
                                    <div className="bg-accent/30">
                                        {group.children.map((child) => {
                                            ...
                                            return (<button key={child.id} onClick={() => handleNav(child.id)} className={cn('w-full flex items-center gap-2.5 pl-10 pr-4 py-2 text-sm transition-colors', childActive ? 'bg-indigo-50 text-indigo-700 font-semibold dark:bg-indigo-900/30 dark:text-indigo-300' : 'text-muted-foreground hover:bg-accent')}>
