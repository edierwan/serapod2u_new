...
import { Calculator, Search, ChevronRight, ChevronDown, X, Menu as MenuIcon } from 'lucide-react'
import {
    financeNavGroups,
    getAllFinanceNavItems,
    getFinanceBreadcrumb,
    type FinanceNavGroup,
    type FinanceNavChild,
} from '@/modules/finance/financeNav'
import { cn } from '@/lib/utils'

// ── Props ────────────────────────────────────────────────────────

interface FinanceTopNavProps {
    /** The current SPA view id (e.g. 'finance/gl/journals') */
    currentView: string
...
export default function FinanceTopNav({ currentView, onNavigate }: FinanceTopNavProps) {
    ...
    const isChildActive = useCallback(
        (child: FinanceNavChild) => child.id === currentView,
        [currentView]
    )

    // ── Breadcrumb ────────────────────────────────────────────────

    const breadcrumbs = useMemo(() => getFinanceBreadcrumb(currentView), [currentView])

    // ── Search ────────────────────────────────────────────────────

    const allItems = useMemo(() => getAllFinanceNavItems(), [])
        ...
        <div className="sticky top-0 z-40 bg-card border-b border-border print:hidden">
            ...
            <div className="flex items-center h-12 px-3 gap-2">
                ...
                <div className="flex items-center gap-1.5 shrink-0 mr-1">
                    <div className="flex items-center gap-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 px-2.5 py-0.5 rounded text-sm font-semibold">
                ...
                <div ref={dropdownRef} className="hidden md:flex items-center flex-1 min-w-0 relative">
                        ...
                        {financeNavGroups.map((group) => {
                            ...
                            return (
                                <button
                                    ...
                                    aria-haspopup="true"
                                    aria-expanded={open}
                                    aria-controls={`finance-dropdown-${group.id}`}
                    ...
                    {/* ── Dropdown panel ── */}
                    {openGroupId && (() => {
                        const group = financeNavGroups.find((g) => g.id === openGroupId)
                        if (!group) return null

                        return (
                            <div
                                id={`finance-dropdown-${group.id}`}
                                role="menu"
