...
import { Settings, Search, ChevronRight, ChevronDown, X, Menu as MenuIcon } from 'lucide-react'
import {
    settingsNavGroups,
...
interface SettingsTopNavProps {
    /** The current SPA view id (e.g. 'settings/profile') */
    currentView: string
...
export default function SettingsTopNav({ currentView, onNavigate, roleLevel }: SettingsTopNavProps) {
    ...
    const visibleGroups = useMemo(() => {
        return settingsNavGroups.filter((group) => {
    ...
    const isChildActive = useCallback(
        (child: SettingsNavChild) => child.id === currentView,
        [currentView]
    )

    // ── Breadcrumb ────────────────────────────────────────────────

    const breadcrumbs = useMemo(() => getSettingsBreadcrumb(currentView), [currentView])

    // ── Search ────────────────────────────────────────────────────

    const allItems = useMemo(() => getAllSettingsNavItems(), [])
        ...
        <div className="sticky top-0 z-40 bg-card border-b border-border print:hidden">
            ...
            <div className="flex items-center h-12 px-3 gap-2">
                ...
                <div ref={dropdownRef} className="hidden md:flex items-center flex-1 min-w-0 relative">
                        ...
                        {visibleGroups.map((group) => {
                            ...
                            return (
                                <button
                                    ...
                                    aria-haspopup="true"
                                    aria-expanded={open}
                                    aria-controls={`settings-dropdown-${group.id}`}
                    ...
                    {/* ── Dropdown panel ── */}
                    {openGroupId && (() => {
                        const group = visibleGroups.find((g) => g.id === openGroupId)
                        if (!group) return null

                        return (
                            <div
                                id={`settings-dropdown-${group.id}`}
                                role="menu"
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                childActive
                                                    ? 'bg-slate-100 text-slate-900 font-semibold dark:bg-slate-800/50 dark:text-slate-100'
