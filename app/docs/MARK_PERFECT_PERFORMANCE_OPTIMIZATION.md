...
**Impact:**
- Removed `code`, `sequence_number`, `last_scanned_at`, `is_buffer` from case codes query
...
**Why:** The API doesn't use sequence order for any business logic. Removing ORDER BY allows Postgres to use faster index scan strategies.
...
- [ ] Apply database indexes: `psql < migrations/030_mark_perfect_performance_indexes.sql`
...
- All business logic, validations, and error messages remain **exactly the same**
