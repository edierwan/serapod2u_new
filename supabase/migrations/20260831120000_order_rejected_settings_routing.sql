-- Order reject/cancel: follow Notification Types routing (one first hop),
-- and send that message only to the order creator. Later hops (SMS/Email)
-- are left to the outbox worker fallback after the first hop fails.
...
BEGIN
  ...
  IF v_event_code = 'order_rejected' THEN
    ...
    -- Treat a missing/NULL settings row the same as explicitly disabled
    -- (fail closed). The outbox worker never re-checks
    -- notification_settings.enabled on its own, so this is the only gate --
    -- without COALESCE, an org with no row here silently got default
    -- whatsapp-only routing instead of no notification at all.
