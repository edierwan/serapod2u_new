-- Migration: Add Inventory Allocation Functions for D2H and S2D Orders
-- Description: Implements allocation/release logic for D2H and S2D orders
-- 
-- Flow:
-- 1. When order is created (submitted status): Allocate inventory (increase quantity_allocated)
-- 2. When order is approved: Release allocation and deduct from on_hand (decrease quantity_allocated, decrease quantity_on_hand)
-- 3. When order is cancelled: Release allocation (decrease quantity_allocated)
--
-- Formula: Available = On Hand - Allocated

-- ============================================================================
-- Function 1: Allocate Inventory for Order
-- ============================================================================
-- Called when D2H or S2D order is created and submitted
-- Increases quantity_allocated to reserve stock for the order
...
COMMENT ON FUNCTION public.allocate_inventory_for_order IS 'Allocates inventory (increases quantity_allocated) when D2H or S2D order is created. This reserves stock for the order.';


-- ============================================================================
-- Function 2: Release Allocation for Order
-- ============================================================================
-- Called when order is cancelled or needs to free up allocated inventory
-- Decreases quantity_allocated to make stock available again
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.stock_movements (
            ...
            v_inventory_org_id,
            v_order.buyer_org_id,
            -v_item.qty, -- Negative for tracking deallocated amount
            v_current_allocated,
...
COMMENT ON FUNCTION public.release_allocation_for_order IS 'Releases allocated inventory (decreases quantity_allocated) when order is cancelled. This makes stock available again.';


-- ============================================================================
-- Update orders_approve function to handle allocation
-- ============================================================================
-- When order is approved:
-- 1. Release allocation (decrease quantity_allocated)
-- 2. Deduct from on_hand (decrease quantity_on_hand)
...
COMMENT ON FUNCTION public.orders_approve IS 'Approves order. For D2H/S2D: releases allocation, deducts seller inventory, and adds to buyer inventory.';
