-- order_approved must follow Notification Types the same way order_rejected does:
-- queue only the first hop (SMS Only → SMS). Do not unnest WhatsApp+SMS+Email.
-- Settings and SMS provider may live on HQ even when orders.company_id is another org.
...
DECLARE
  v_event_code TEXT;
  v_company_id UUID;
  v_config_org_id UUID;
  v_settings_org_id UUID;
...
BEGIN
  ...
  IF v_event_code IN ('order_rejected', 'order_approved') THEN
    SELECT ns.enabled, ns.channels_enabled, ns.recipient_config, ns.org_id
    INTO v_setting_enabled, v_channels_enabled, v_recipient_config, v_config_org_id
    ...
    IF v_setting_enabled IS NULL THEN
      ...
      INTO v_setting_enabled, v_channels_enabled, v_recipient_config, v_config_org_id
      FROM notification_settings ns
      JOIN organizations o ON o.id = ns.org_id
      WHERE ns.event_code = v_event_code
    ...
    v_settings_org_id := COALESCE(v_config_org_id, v_company_id);
    ...
    SELECT p.channel, p.org_id
    INTO v_first_channel, v_provider_org_id
    FROM unnest(v_chain) WITH ORDINALITY AS c(channel, ord)
    JOIN notification_provider_configs p
      ON p.channel = c.channel
     AND p.is_active IS TRUE
     AND (
       p.org_id = v_settings_org_id
       OR p.org_id IN (
         SELECT id FROM organizations
         WHERE org_type_code = 'HQ' AND is_active IS TRUE
       )
     )
    ORDER BY c.ord,
      CASE WHEN p.org_id = v_settings_org_id THEN 0 ELSE 1 END
    LIMIT 1;

    IF v_first_channel IS NULL THEN
      v_first_channel := v_chain[1];
      v_config_org_id := v_settings_org_id;
    ELSE
      v_config_org_id := COALESCE(v_provider_org_id, v_settings_org_id);
    END IF;

    PERFORM public.queue_notification(
      COALESCE(v_config_org_id, v_company_id),
      v_event_code,
      v_first_channel,
      CASE
...
COMMENT ON FUNCTION public.trigger_order_notification() IS
  'Queues notifications only when order status actually changes. order_approved and order_rejected queue the first hop from notification_settings (HQ fallback for settings/provider). Fallback hops are handled by the outbox worker.';
