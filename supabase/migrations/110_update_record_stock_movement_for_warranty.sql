-- Migration: Update record_stock_movement to support warranty_bonus
-- Description: Update the function to handle warranty_bonus movement type correctly (ensure it's treated as an addition)
...
BEGIN
    ...
    SET 
        ...
        updated_at = NOW(),
        -- Update average cost if unit cost is provided and it's an addition
        -- For warranty_bonus, unit cost is 0, so it will lower the average cost
        average_cost = CASE 
            WHEN p_quantity_change > 0 AND p_unit_cost IS NOT NULL THEN
                ((quantity_on_hand * COALESCE(average_cost, 0)) + (p_quantity_change * p_unit_cost)) / (quantity_on_hand + p_quantity_change)
            ELSE average_cost
        END
