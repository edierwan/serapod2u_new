-- Optimize mark_batch_as_printed to handle large batches without timeouts
-- This version uses batch processing with controlled commit intervals

-- Drop existing function first (return type changed from void to jsonb)
DROP FUNCTION IF EXISTS mark_batch_as_printed(uuid);
...
COMMENT ON FUNCTION mark_batch_as_printed IS 'Updates batch status to printing and marks all associated QR codes as printed. Handles large batches with chunked updates to prevent timeouts.';
