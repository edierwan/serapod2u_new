-- Migration: Update Order Status Flow
-- Description: Add new statuses and update approval logic for D2H/S2D orders

-- 1. Add new enum values
ALTER TYPE public.order_status ADD VALUE IF NOT EXISTS 'warehouse_packed';
ALTER TYPE public.order_status ADD VALUE IF NOT EXISTS 'shipped_distributor';

-- 2. Update orders_approve function
...
BEGIN
  ...
  -- Allow approval from submitted OR warehouse_packed
  IF v.status NOT IN ('submitted', 'warehouse_packed') THEN 
    RAISE EXCEPTION 'Order must be in submitted or warehouse_packed status'; 
  END IF;
  ...
  IF v.order_type IN ('D2H', 'S2D') THEN
    v_new_status := 'shipped_distributor';
  ELSE
