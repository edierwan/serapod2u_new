-- Migration: Update D2H/S2D Document Flow
-- 1. Create Sales Order (SO) on Order Submission
-- 2. Create Delivery Order (DO) and Invoice on Order Approval (instead of PO)

-- Part 1: Function to generate SO on submission
CREATE OR REPLACE FUNCTION public.handle_new_order_documents()
RETURNS TRIGGER AS $$
DECLARE
    v_so_doc_no text;
BEGIN
    -- Only for D2H and S2D orders
    IF NEW.order_type NOT IN ('D2H', 'S2D') THEN
        RETURN NEW;
    END IF;
    ...
    IF NEW.status = 'submitted' AND (OLD.status IS NULL OR OLD.status != 'submitted') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM public.documents WHERE order_id = NEW.id AND doc_type = 'SO') THEN
            v_so_doc_no := 'SO-' || NEW.order_no;
            ...
            INSERT INTO public.documents (
                ...
                v_so_doc_no,
                'pending', 
                NEW.seller_org_id, -- Issued BY Seller
                NEW.buyer_org_id,  -- Issued TO Buyer
                NEW.created_by,
                now(),
                now()
            );
...
-- Create Trigger
DROP TRIGGER IF EXISTS on_order_submit_generate_so ON public.orders;
CREATE TRIGGER on_order_submit_generate_so
    AFTER INSERT OR UPDATE OF status ON public.orders
    FOR EACH ROW
    EXECUTE FUNCTION public.handle_new_order_documents();


-- Part 2: Update orders_approve to generate DO and Invoice instead of PO for D2H/S2D
