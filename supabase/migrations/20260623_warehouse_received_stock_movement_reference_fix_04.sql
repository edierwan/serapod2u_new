-- ============================================================================
-- Warehouse Receive: stock_movements reference_type fix (04)
-- ----------------------------------------------------------------------------
-- BUG: post_warehouse_receipt (migration 02) called record_stock_movement with
--      p_reference_type => 'warehouse_receipt', which is NOT permitted by
--      stock_movements_reference_type_check:
--        CHECK (reference_type = ANY (ARRAY[
--          'manual','order','transfer','adjustment',
--          'purchase_order','return','campaign']))
--      => "new row for relation \"stock_movements\" violates check constraint
--          stock_movements_reference_type_check"
--
-- FIX (safest, no constraint/data change): reuse the existing, accurate value
--      'order' — a warehouse receipt posts stock received against an order, and
--      the legacy full-receive worker already uses reference_type='order'. The
--      movement therefore stays consistent with existing data and reporting.
--
--      Receipt traceability is fully preserved:
--        * stock_movements.reference_id  = order_id   (order link, matches worker)
--        * stock_movements.reference_no  = order_no
--        * stock_movements.notes         = 'Warehouse receipt <WR-no> (<type>)'
--        * warehouse_receipt_items.stock_movement_id = the movement id (line link)
--
--      No change to stock_movements_reference_type_check and NO existing stock
--      movement rows are modified or deleted.
--
-- This migration ONLY does CREATE OR REPLACE on the RPC. It does not touch
-- migrations 01/02/03 or their objects' data.
--
-- ALTERNATIVE (not used): if a dedicated 'warehouse_receipt' reference type is
-- ever desired, extend the constraint instead, e.g.:
...
--     CHECK (reference_type = ANY (ARRAY['manual','order','transfer','adjustment',
--            'purchase_order','return','campaign','warehouse_receipt']));
-- and set p_reference_type => 'warehouse_receipt' below. This migration keeps the
-- lower-risk 'order' approach.
--
-- ROLLBACK NOTES (manual): re-apply migration 02's version of
--   post_warehouse_receipt (the only difference is the reference_type/id/no and
--   the added per-variant error context). No data rollback required.
-- ============================================================================
