-- Serapp may issue a DO document at warehouse-accept time, before HQ
-- calls orders_approve. Keep approval inventory/SO/Invoice behaviour, but
-- never create a second DO (or SO / Invoice) for the same order.
...
COMMENT ON FUNCTION public.orders_approve(uuid) IS
  'Approve submitted order, fulfill inventory for D2H/S2D, and ensure SO/DO/Invoice (or PO) exist without duplicating docs pre-created by Serapp.';
