-- Include the order creator name in order notification payloads.
-- SMS templates can use {{created_by}} instead of {{customer_name}}.
...
BEGIN
  ...
  SELECT COALESCE(NULLIF(TRIM(u.full_name), ''), u.email, 'Unknown')
...
COMMENT ON FUNCTION public.trigger_order_notification() IS 'Queues enriched notifications on order status change (submitted, approved, closed, cancelled). Includes created_by (user full name) for SMS templates.';
