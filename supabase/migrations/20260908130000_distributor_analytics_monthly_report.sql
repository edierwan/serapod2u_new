-- Monthly Distributor Analytics management report, aggregated inside the database.
--
-- Replaces the previous model where /api/reporting/distributors/report pulled a
-- rolling window of orders WITH their nested order_items, aggregated them in the
-- route, and then also returned the raw order population and every distributor
-- to the browser. This function returns one compact JSON report per reporting
-- month, distributor scope and status scope.
--
-- SECURITY INVOKER is intentional and matches reporting_product_analytics:
-- orders / order_items / organizations / product_variants RLS stays
-- authoritative, so the function can never widen what the signed-in manager is
-- already allowed to read.
--
-- Reporting model (identical to lib/reporting/distributor-analytics.ts):
...
--     previous calendar month, across year boundaries;
--   * eligible orders are order_type D2H whose buyer organisation is a DIST,
--     bucketed on orders.created_at — unchanged from the report this replaces;
--   * Order Value is summed from order_items, never from an order header;
--   * when p_status is NULL every status is included, which is the previous
--     report's behaviour and is what makes Order Processing Health measurable.
--
-- "First ever order" and dormancy are computed from the distributor's whole
-- eligible history, which is the entire reason this belongs in SQL: the report
-- window alone cannot tell a genuinely new distributor from a returning one.
--
...
COMMENT ON FUNCTION public.reporting_distributor_order_periods() IS
  'Reporting months containing eligible D2H distributor orders, in Asia/Kuala_Lumpur; honors orders RLS.';
...
BEGIN
  ...
  top_products AS (
    SELECT oi.variant_id,
           max(pv.product_id)      AS product_id,
