-- Migration: Fix Order Movement (Remove Double Deduction)
-- Description: Update orders_approve to NOT log 'order_fulfillment' movement for the seller.
--              This prevents the "double deduction" appearance in Movement History.
--              The 'Alloc' movement serves as the record of stock usage.
--              The 'transfer_in' movement for the buyer is preserved.
...
BEGIN
  ...
  IF v.order_type IN ('D2H', 'S2D') THEN
    FOR v_item IN SELECT * FROM public.order_items WHERE order_id = v.id LOOP
        ...
        -- REMOVED: Log seller inventory movement (order_fulfillment)
        -- We do NOT log the deduction here to avoid "double deduction" appearance in history.
        -- The 'Alloc' movement (created at order submission) serves as the visual record of stock usage.
        -- The actual On Hand deduction happens silently here.
