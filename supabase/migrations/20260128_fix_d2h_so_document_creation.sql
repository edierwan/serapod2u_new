...
--
-- Key rules:
-- - doc_no for SO = orders.order_no (clean, no "SO-" prefix)
-- - display_doc_no auto-generated by trigger
-- - All inserts use NOT EXISTS guards (idempotent)
--
...
BEGIN
  ...
  IF v.order_type IN ('D2H', 'S2D') THEN
    FOR v_item IN SELECT * FROM public.order_items WHERE order_id = v.id LOOP
        ...
        INSERT INTO public.stock_movements (
            ...
            variant_id, from_organization_id, to_organization_id,
            quantity_change, quantity_before, quantity_after,
            company_id, created_by, created_at, notes
        ) VALUES (
            'transfer_in', 'order', v.id, v.order_no,
            v_item.variant_id, v_inventory_org_id, v.buyer_org_id,
  ...
  -- =========================================================================
  -- Document Generation Logic (idempotent with NOT EXISTS guards)
  -- =========================================================================
  IF v.order_type IN ('D2H', 'S2D') THEN
      -- Insert SO document: doc_no = order_no (clean, no "SO-" prefix)
      IF NOT EXISTS (SELECT 1 FROM documents WHERE order_id = v.id AND doc_type = 'SO') THEN
      ...
      -- Insert DO document
      IF NOT EXISTS (SELECT 1 FROM documents WHERE order_id = v.id AND doc_type = 'DO') THEN
          INSERT INTO public.documents (
            company_id, order_id, doc_type, doc_no, status,
            issued_by_org_id, issued_to_org_id, created_by, created_at, updated_at
          ) VALUES (
      ...
      IF NOT EXISTS (SELECT 1 FROM documents WHERE order_id = v.id AND doc_type = 'INVOICE') THEN
          INSERT INTO public.documents (
            company_id, order_id, doc_type, doc_no, status,
            issued_by_org_id, issued_to_org_id, created_by, created_at, updated_at
          ) VALUES (
  ...
  ELSE
      ...
      IF NOT EXISTS (SELECT 1 FROM documents WHERE order_id = v.id AND doc_type = 'PO') THEN
