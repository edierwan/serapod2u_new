...
-- LOCK IMPACT  : ALTER TABLE ... ADD CONSTRAINT on stock_movements takes an
--                ACCESS EXCLUSIVE lock and re-validates every row. On a large
--                stock_movements table this blocks reads and writes for the
--                duration -- run it in a maintenance window. See
--                ROLLBACK_AND_RECOVERY.md "Lock-sensitive steps".
...
--                The pre-check DO block aborts if duplicates would violate it.
-- -----------------------------------------------------------------------------
